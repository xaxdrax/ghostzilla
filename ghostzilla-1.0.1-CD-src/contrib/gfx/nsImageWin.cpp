  ...
  if (TheHDC != NULL){
    if (mSizeImage > 0){
       if (mAlphaDepth == 8) {
	    ...
	    // [ghzil] {
		__declspec(dllimport) void nsImageWin_CreateDDB_14(void *, void *, signed char *, int, int, int, int, unsigned char *);
		nsImageWin_CreateDDB_14( this, mAlphaBits, &mAlphaDepth, mNaturalHeight, mNaturalWidth,
			mRowBytes, mNumBytesPixel, mImageBits);
	    // } [ghzil]
